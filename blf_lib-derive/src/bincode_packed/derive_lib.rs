...
pub fn derive_encode_inner(input: TokenStream, alignment: usize) -> Result<TokenStream> {
    ...
    match body {
        Body::Struct(body) => {
            derive_struct::DeriveStruct {
                ...
                .generate_encode(&mut generator, alignment)?;
        }
        // Body::Enum(body) => {
...
pub fn derive_decode_inner(input: TokenStream, alignment: usize) -> Result<TokenStream> {
    let parse = Parse::new(input)?;
    let (mut generator, attributes, body) = parse.into_generator();
    let attributes = attributes
        .get_attribute::<ContainerAttributes>()?
    ...
    match body {
        Body::Struct(body) => {
            derive_struct::DeriveStruct {
                ...
                .generate_decode(&mut generator, alignment)?;
        }
        // Body::Enum(body) => {
        //     derive_enum::DeriveEnum {
        //         variants: body.variants,
